...
def make_app(
    ...
    *,
    ...
    add_auth(app, config, deploy_ui=deploy_ui)
    ...
    if deploy_ui:
        ...
        pn.serve(
            ...
            port=31477,
            allow_websocket_origin=["127.0.0.1:31476", "127.0.0.1:31477"],
            address="127.0.0.1",
            show=False,
            threaded=True,
        )
    ...
    async def base_redirect() -> Response:
        return RedirectResponse(
            "/ui" if deploy_ui else "/docs", status_code=status.HTTP_303_SEE_OTHER
        )
...
def add_auth(app: FastAPI, config: Config, *, deploy_ui: bool) -> None:
    auth = config.auth()
