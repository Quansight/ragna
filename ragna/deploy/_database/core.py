...
class Database:
    def __init__(self, config):
        components = urlsplit(config.api.database_url)
        if components.scheme == "sqlite":
            connect_args = dict(check_same_thread=False)
        else:
            connect_args = dict()
        engine = create_engine(config.api.database_url, connect_args=connect_args)
    ...
    def session(self) -> Iterator[Session]:
        with self._make_session() as session:  # type: ignore[attr-defined]
            yield session
    ...
    def add_document(
        ...
        *,
        ...
        session.add(
            orm.Document(
                id=document.id,
                user_id=self._get_user_id(session, user),
                name=document.name,
                metadata_=metadata,
            )
        )
    ...
    def _orm_to_schema_document(self, document: orm.Document) -> schemas.Document:
    ...
    def get_document(
        ...
        document = session.execute(
            select(orm.Document).where(
                (orm.Document.user_id == self._get_user_id(session, user))
                & (orm.Document.id == id)
            )
        ).scalar_one_or_none()
        return self._orm_to_schema_document(document), document.metadata_
    ...
    def _orm_to_schema_chat(self, chat: orm.Chat) -> schemas.Chat:
        documents = [
        ...
        messages = [
            schemas.Message(
                ...
                sources=[
                    schemas.Source(
                        id=source.id,
                        document=self._orm_to_schema_document(source.document),
    ...
    def get_chats(self, session: Session, *, user: str) -> list[schemas.Chat]:
        return [
            self._orm_to_schema_chat(chat)
            for chat in session.execute(
                select(orm.Chat).where(
                    orm.Chat.user_id == self._get_user_id(session, user)
                )
            )
            .scalars()
            .all()
    ...
    def _get_orm_chat(self, session: Session, *, user: str, id: uuid.UUID) -> orm.Chat:
    ...
    def get_chat(self, session: Session, *, user: str, id: uuid.UUID) -> schemas.Chat:
        return self._orm_to_schema_chat(self._get_orm_chat(session, user=user, id=id))

    def _schema_to_orm_source(
        self, session: Session, *, source: schemas.Source
    ) -> orm.Source:
    ...
    def _schema_to_orm_message(
        self, session: Session, *, chat_id: uuid.UUID, message: schemas.Message
    ) -> orm.Message:
        ...
        if orm_message is None:
            orm_message = orm.Message(
                ...
                role=message.role,
                sources=[
                    self._schema_to_orm_source(session, source=source)
                    for source in message.sources
                ],
                timestamp=message.timestamp,
            )
    ...
    def update_chat(self, session: Session, *, user: str, chat: schemas.Chat) -> None:
        orm_chat = self._get_orm_chat(session, user=user, id=chat.id)

        orm_chat.prepared = chat.prepared
        orm_chat.messages = [
            self._schema_to_orm_message(session, chat_id=chat.id, message=message)
            for message in chat.messages
        ]
